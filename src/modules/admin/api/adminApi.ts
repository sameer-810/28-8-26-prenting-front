...
export const adminApi = {
  ...
  overview: () => adminApiClient.get("/overview").then((r) => r.data.data as AdminOverview),
  ...
  familyDetail: (id: string) =>
    adminApiClient.get(`/families/${id}`).then((r) => r.data.data as AdminFamilyDetail),
  ...
  setPlan: (id: string, planCode: PlanCode, note: string) =>
    adminApiClient.patch(`/families/${id}/plan`, { planCode, note }).then((r) => r.data.data),

  setActive: (id: string, isActive: boolean, note: string) =>
    adminApiClient.patch(`/families/${id}/active`, { isActive, note }).then((r) => r.data.data),

  curriculum: () =>
    adminApiClient.get("/curriculum").then((r) => r.data.data as CurriculumCoverageRow[]),

  listAdmins: () => adminApiClient.get("/admins").then((r) => r.data.data as PlatformAdminRow[]),
  ...
  }) => adminApiClient.post("/admins", input).then((r) => r.data.data as PlatformAdminRow),
};
