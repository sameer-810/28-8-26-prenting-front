...
const PARENT = { email: "demo.parent@parentai.app", password: "ParentAI-Demo-2026" };
...
check("it says plainly this is not the parent sign-in", /not the parent sign-in/i.test(door));
check("no platform data before signing in", !/Households|Entitled MRR/.test(door));
check(
...
check("a wrong password shows the server's message", /Could not sign in/i.test(await sceneText()));
...
check("the signed-in staff member and their role are shown", /superadmin|support/.test(overview));
check("household, child and parent counts render", /Households/.test(overview) && /Children/.test(overview) && /Parents/.test(overview));
check(
  "entitled MRR is labelled as entitlement, not revenue",
  /Entitled MRR/.test(overview) && /not cash received/i.test(overview),
  overview.match(/₹[\d,]+/)?.[0],
);
check("AI health is broken down per operation", /concept/.test(overview) && /degraded/i.test(overview));
check(
...
check("search narrows the list", /Sharma/.test(filtered) && !/GateFam/.test(filtered));

console.log("\n=== 6. One household in detail ===");
await page.getByRole("button", { name: /^Sharma/ }).first().click();
await page.waitForTimeout(3000);
const detail = await sceneText();
check("the household opens", /Sessions done/.test(detail));
check("the URL follows the household", /\/admin\/households\//.test(page.url()), page.url());
check("children are listed with their progress", /Children/.test(detail) && /streak/i.test(detail));
check("parents are listed", /Parents/.test(detail) && /@/.test(detail));
check("the consent record is shown", /Consent/.test(detail) && /policy version/i.test(detail));
check(
...
const role = await page.evaluate(() => {
  try {
    return JSON.parse(sessionStorage.getItem("parentai-admin") || "{}")?.state?.admin?.role || null;
  } catch {
    return null;
  }
});
check("the staff role is readable from the session", Boolean(role), role || "none");

if (role === "superadmin") {
  check("a superadmin sees the plan controls", /Superadmin actions/.test(detail));
  check("a reason is required alongside a plan change", /Reason \(recorded/.test(detail));
  ...
  check("disabling also demands a reason first", /Say why/.test(await sceneText()));
} else {
  check("a support account does not see the plan controls", !/Superadmin actions/.test(detail));
...
check("a total is stated", /active topics/.test(curriculum), curriculum.match(/[\d,]+ active topics/)?.[0]);
check(
...
check("staff accounts are listed with their roles", /superadmin|support/.test(staff));
check("who can open the console is visible to every staff member", /Who can open this console/i.test(staff));
if (role === "superadmin") {
...
await page.goto(`${base}/admin`, { waitUntil: "networkidle" });
await page.waitForTimeout(2500);
check("staff session survived the detour through the parent app", /Overview|Households/.test(await sceneText()));

await page.getByRole("button", { name: "Sign out" }).click();
await page.waitForTimeout(1500);
check("signing out returns to the staff login", /not the parent sign-in/i.test(await sceneText()));
...
const real = consoleErrors.filter(
  (e) => !/Download the React DevTools|deprecated|findDOMNode|status of 40[13]/i.test(e),
);
