...
check(
  "the plan state is explained in words",
  /Renews on|Free trial|subscription has ended|couldn't take/i.test(settings),
  settings.match(/(Renews on [^\n]+|Free trial[^\n]*)/)?.[0],
);
check(
  "the study window is editable",
  /When you study/.test(settings) &&
    /From/.test(settings) &&
    /Until/.test(settings),
);
check(
  "reminder toggles are present",
  /Evening study nudge/.test(settings) && /Weekly summary/.test(settings),
);
check(
...
const after = await page
  .getByRole("switch", { name: /Weekly summary/i })
...
check(
  "all three paid plans are listed",
  /Basic Monthly/.test(plans) &&
    /Family Annual/.test(plans) &&
    /Family Plus/.test(plans),
);
check(
  "prices come from the API, formatted for India",
  /₹499/.test(plans) && /₹4,999/.test(plans),
  plans
    .match(/₹[\d,]+/g)
    ?.slice(0, 3)
    .join(" "),
...
check(
  "the consent record is shown with its date and version",
  /YOUR CONSENT/.test(privacy) && /policy version/i.test(privacy),
);
check(
  "deletion is owner-gated and explained",
  /Delete everything/.test(privacy) && /cannot be undone/i.test(privacy),
);
...
check(
  "the data export downloads",
  exported.suggestedFilename().endsWith(".json"),
...
for (const [name, url] of [
  ...
  const report = await page.evaluate(() => {
    ...
    const imagesWithoutAlt = Array.from(
      ...
      (i) =>
        !i.getAttribute("alt") && i.getAttribute("role") !== "presentation",
    );
    ...
    return {
      ...
      statelessRoles: [
        ...new Set(stateless.map((e) => e.getAttribute("role"))),
      ],
    };
  });
...
check(
  ...
  totalNameless === 0
    ...
    : audited
        .filter((a) => a.nameless)
        .map((a) => `${a.name}: ${a.nameless} (${a.namelessTags.join(", ")})`)
        .join(" | "),
);
check(
...
check(
  ...
  totalStateless === 0
    ...
    : audited
        .filter((a) => a.stateless)
        .map((a) => `${a.name}: ${a.stateless} ${a.statelessRoles.join(",")}`)
        .join(" | "),
);
...
const reachable = await page.evaluate(async () => {
  ...
  for (let i = 0; i < 40; i += 1) {
    ...
    if (el && el !== document.body)
      seen.add(el.getAttribute("aria-label") || el.tagName);
...
check(
  "the settings screen is reachable by keyboard",
  focusable >= 15,
  `${focusable} of 25 tab stops landed on something focusable`,
);
...
check(
  "the app renders normally under prefers-reduced-motion",
  /Aarav/.test(rmText),
);
